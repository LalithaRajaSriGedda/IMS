...
class productClass:
    def __init__(self,root):
        ...
        lbl_productname=Label(self.root,text="Name",font=("goudy old style",18),bg="white").place(x=30,y=160)
        ...
        #====searchFrame=========
        SearchFrame=LabelFrame(self.root,text="Serach Product",font=("goudy old style",12,"bold"),bd=2,relief=RIDGE,bg="white")
...
#===================================================================================================================
    ...
    def add(self):
        ...
        cur = con.cursor()
        try:
            print("Connected to database: ", con)
    ...
    def get_data(self, event):
        try:
            f = self.product_table.focus()  # Get selected item ID
            if not f:
                messagebox.showerror("Error", "No record selected!", parent=self.root)
                return
        
            content = self.product_table.item(f)  # Get the item dictionary
            row = content.get('values', ())  # Ensure row is a tuple

            print("DEBUG: Selected Row Data:", row)  # Print row for debugging

        # Check if row has enough values before assigning
            if not row or len(row) < 7:
                messagebox.showerror("Error", "Invalid data selected!", parent=self.root)
                return
        ...
        try:
            if self.var_pid.get() == "":
                ...
                if row is None:
                    ...
                    cur.execute("UPDATE product SET Category=?, Supplier=?, Price=?, qty=?, status=? WHERE pid=?", (
                    self.var_cat.get(),
                    self.var_sup.get(),
                    self.var_price.get(),
                    self.var_qty.get(),
                    self.var_status.get(),
                    self.var_pid.get(),  # Corrected this line
                ))
                con.commit()
            ...
            if self.var_pid.get() == "" or self.var_name.get() == "":
                ...
                if row is None:
                    ...
                    if op==True:
                        ...
                        con.commit()
                        messagebox.showinfo("Delete", "Product Deleted successfully", parent=self.root)  # Corrected this line
                        self.clear()
        except Exception as ex:
            messagebox.showerror("Error", f"Error due to: {str(ex)}", parent=self.root)
