...
#===================================================================================================================
    def add(self):
        ...
        try:
            if self.var_emp_id.get()=="" or self.var_name.get()=="":
                ...
                row=cur.fetchone()
                if row!=None:
                    messagebox.showerror("Error","This Employee ID already assigned ,try diferent",parent=self.root)
                else:
    ...
    def get_data(self, event):
        ...
        row = self.EmployeeTable.item(selected_item, "values")
            ...
            if self.var_emp_id.get() == "" or self.var_name.get() == "":
                ...
                if row is None:
                    ...
                    if op:
                        ...
                        con.commit()
                        messagebox.showinfo("Delete", "Employee Deleted successfully", parent=self.root)  # Corrected this line
                        self.clear()
        except Exception as ex:
            messagebox.showerror("Error", f"Error due to: {str(ex)}", parent=self.root)
