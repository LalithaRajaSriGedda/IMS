...
class BillClass:
    def __init__(self,root):
        ...
        #==btn_logout==
        btn_logout=Button(self.root,text="Logout",command=self.logout,font=("times new roman",15,"bold"),bg="blue",cursor="hand2").place(x=1150,y=10,height=50,width=150)
        #==clock==
        ...
        txt_search=Entry(Product_Frame2,textvariable=self.var_search,font=("times  new roman",15),bg="lightyellow").place(x=138,y=47,width=150,height=22)
        btn_search=Button(Product_Frame2,text="Search",command=self.search,font=("goudy oid style",15),bg="#2196f3",fg="white",cursor="hand2").place(x=285,y=45,width=100,height=25)
        btn_show_All=Button(Product_Frame2,text="Show All",command=self.show,font=("goudy oid style",15),bg="#083531",fg="white",cursor="hand2").place(x=285,y=10,width=100,height=25)
        ...
        lbl_note=Label(Product_Frame1,text="Note: 'Enter 0 quantity to remove from the cart'",font=("goudy old style",12),anchor='w',bg="white",fg="red").pack(side=BOTTOM,fill=X)
        ...
        self.lbl_net_pay=Label(billMenuFrame,text='Pay amount\n[0]',font=('goudy old style',15,'bold'),bg="#607d8b",fg='white')
        ...
        #=============footer==========
        footer=Label(self.root,text="IMS-Inventory Management System |Developed by Lalitha for any technical issues conatct:630XXXXXXX",font=("times new roamn",11),bg="#4d636d",fg="white").pack(side=BOTTOM,fill=X)
        self.show()
...
#=================All Functions===================
    ...
    def search(self):
        ...
        try:
            if self.var_search.get() == "":
                ...
                query = "SELECT * FROM product WHERE name LIKE ? AND status = ?"
                cur.execute(query, ('%' + self.var_search.get() + '%', 'Active'))
    ...
    # =========== Update Cart ===========
        present = False
        for index_, row in enumerate(self.cart_list):
            if row[0] == self.var_pid.get():  # If product exists in cart
                new_qty = int(row[3]) + qty  # Update quantity
                if new_qty > stock:  # Prevent adding more than available stock
                    messagebox.showerror("Error", "Not enough stock available!", parent=self.root)
                    return
            
                new_price = new_qty * price  # Update price
                self.cart_list[index_] = [row[0], row[1], new_price, new_qty, stock - new_qty]  # Update cart data
                present = True
                break  # Exit loop after updating

        if not present:
            self.cart_list.append([self.var_pid.get(), self.var_pname.get(), total_price, qty, stock - qty])  # Add new item if not in cart

    # Update stock after adding to cart
        new_stock = stock - qty
        self.var_stock.set(str(new_stock))  # Update stock display
        self.lbl_inStock.config(text=f"In Stock [{new_stock}]")  # Update label
    ...
    def show_cart(self):
        ...
        try:
            #self.product_Table=ttk.Treeview(Product_Frame3,columns=("pid","name","price","qty","status"),yscrollcommand=scrolly.set,xscrollcommand=scrollx.set)
            self.product_Table.delete(*self.Cart_Table.get_children())
            for row in self.cart_list:
    ...
    def bill_top(self):
        ...
        bill_top_temp=f'''
\t\tXYZ-Inventory
\t Phone No.6300XXXXXX, Delhi-125001
{str("="*47)}
Customer Nmae:{self.var_cname.get()}
