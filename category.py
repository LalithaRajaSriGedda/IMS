...
class categoryClass:
    def __init__(self,root):
        ...
        btn_delete=Button(self.root,text="DELETE",command=self.delete,font=("goudy old style",15),bg="red",fg="white",cursor="hand2").place(x=540,y=170,width=150,height=30)
        ...
        self.category_table.column("name",width=120)
...
#==========================Functions======================
    def add(self):
        ...
        try:
            if self.var_name.get()=="" or self.var_name.get()=="":
                ...
                row=cur.fetchone()
                if row!=None:
                    messagebox.showerror("Error","Category already present,try diferent",parent=self.root)
                else:
    ...
    def delete(self):
        ...
        cur = con.cursor()
        try:
            if self.var_cat_id.get() == "":  # ✅ Correct variable name
                messagebox.showerror("Error", "Please select category from the list", parent=self.root)
            else:
                cur.execute("Select * from category where cid=?", (self.var_cat_id.get(),))  # ✅ Fixed variable
                row = cur.fetchone()
                if row is None:
                    ...
                    if op:
                        cur.execute("Delete from category where cid=?", (self.var_cat_id.get(),))  # ✅ Fixed
                        con.commit()
